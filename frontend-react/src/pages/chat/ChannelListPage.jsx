...
const ChannelListPage = () => {
  ...
  const approveMember = async (channelId, patientId) => {
    console.log("Approved memebers ", channelId, patientId);
    await ApiService.request(
      "/chat/approve-member",
      "POST",
      { channelId, patientId },
  ...
  let list = [];
  if (isDoctor) {
    console.log("Channels for doctor:", channels);
    if (tab === "approved") {
      list = channels.flatMap((ch) =>
        (ch.members || [])
    ...
    if (tab === "approved") {
      list = channels.filter(
        (ch) =>
          ...
          ch.members.some(
            (m) =>
              String(m.user) === String(user.userID) && m.status === "approved"
          )
      );
    }
    if (tab === "pending") {
      list = channels.filter(
        (ch) =>
          ...
          ch.members.some(
            (m) =>
              String(m.user) === String(user.userID) && m.status === "pending"
          )
      );
    }
    if (tab === "all") {
  ...
  return (
    <div style={{ maxWidth: 700, margin: "28px auto 0 auto" }}>
      ...
      <div className="channel-list-section">
        ...
        {list.length === 0 && (
          <div style={{ color: "#888", margin: "18px 0 0 12px" }}>
            Nothing to show yet.
          </div>
        )}
                ...
                <div className="channel-list-card" key={ch._id + idx}>
                  <div>
                    ...
                    <div className="channel-meta">
                      Patient ID: {ch.patient.user} | approved
                    </div>
                  </div>
                  <div className="channel-actions">
                    ...
                    <button
                      ...
                      onClick={() => removeMember(ch._id, ch.patient.user)}
                    >
                      Remove
                    </button>
                  </div>
              ...
              list.map((ch, idx) => (
                <div className="channel-list-card" key={ch._id + idx}>
                  <div>
                    <div className="channel-info">{ch.name}</div>
                    <div className="channel-meta">
                      Patient ID: {ch.patient.user} | pending
                    </div>
                  </div>
                  <div className="channel-actions">
                    <button
                      className="btn-approve"
                      onClick={() => approveMember(ch._id, ch.patient.user)}
                    >
                      Approve
                    </button>
                    <button
              ...
              list.map((ch) => (
                <div className="channel-list-card" key={ch._id}>
                  ...
                  <div className="channel-actions">
                    ...
                    <button
                      className="btn-approve"
                      onClick={() => removeMember(ch._id, user.userID)}
                    >
                      Leave
                    </button>
                  </div>
            ...
            {tab === "all" &&
              list.map((ch) => {
                const m =
                  Array.isArray(ch.members) &&
                  ch.members.find(
                    (m_) => String(m_.user) === String(user.userID)
                  );
                return (
                  <div className="channel-list-card" key={ch._id}>
                    <div className="channel-info">{ch.name}</div>
