...
//       return res
//         .status(400)
//         .json({ message: "Already requested or joined" });
...
//       recipient: channel.doctor,
//       sender: req.user.id,
//       type: "System",
//       title: "New chat request",
//       message: `A patient requested to join channel ${channel.name}`,
//     });

//     // emit socket event to that doctor (room: user:<doctorId>)
//     req.app.get("io").to(`user:${channel.doctor}`).emit("notification", notif);
...
//       (m) => String(m.user) === String(patientId)
//     );
...
//       message: `Your request for channel ${channel.name} was approved.`,
//     });
...
//       return res
//         .status(404)
//         .json({ message: "User not found in channel" });

//     await channel.save();

//     // if doctor removed the patient, send "rejected/removed" notification
//     if (isDoctor) {
//       const notif = await Notification.create({
//         recipient: patientId,
//         sender: req.user.id,
...
//         message: `Your request for channel ${channel.name} was rejected or you were removed.`,
//       });
...
exports.requestToJoin = async (req, res) => {
  ...
  try {
    ...
    io.to(`user:${channel.doctor}`).emit("notification", notif);
...
exports.cancelRequest = async (req, res) => {
  ...
  try {
    ...
    channel.members = channel.members.filter(
      (m) =>
        !(
          String(m.user) === String(req.user.id) && m.status === "pending"
        )
    );
    await channel.save();
    res.json({ success: true, message: "Request canceled" });
...
exports.approveMember = async (req, res) => {
  const { channelId, patientId } = req.body;
  try {
    console.log("Approving member:", { channelId, patientId });
    const channel = await Channel.findById(channelId);
    if (!channel)
      return res.status(404).json({ message: "Channel not found" });
    ...
    const io = req.app.get("io");
    io.to(`user:${patientId}`).emit("notification", notif);
